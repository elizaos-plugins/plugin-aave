...
export class eModeAction implements Action {
    name = 'AAVE_EMODE';
    ...
    validate(runtime: IAgentRuntime, message: Memory): boolean {
        ...
        return (text.includes('emode') || text.includes('efficiency mode') || 
                text.includes('e mode') || text.includes('e-mode')) && 
               text.includes('aave');
    }
    ...
    async handler(
        ...
        try {
            ...
            const paramsSchema = z.object({
                categoryId: z.number().min(0).max(2).describe('eMode category ID (0=disabled, 1=stablecoins, 2=ETH)'),
                enable: z.boolean().describe('True to enable eMode, false to disable')
            });
            ...
            const categoryId = extractedParams.data.enable ? extractedParams.data.categoryId : 0;
            ...
            if (position.eModeCategory === params.categoryId) {
                const modeText = params.categoryId === 0 ? 'disabled' : `set to category ${params.categoryId}`;
                throw new Error(`Efficiency mode is already ${modeText}`);
            }

            // Validate asset compatibility for eMode categories
            if (params.categoryId > 0) {
                const incompatibleAssets = this.checkAssetCompatibility(position, params.categoryId);
                if (incompatibleAssets.length > 0) {
                    throw new Error(`Cannot enable eMode category ${params.categoryId}. Incompatible assets: ${incompatibleAssets.join(', ')}`);
                }
            }
            ...
            // Get category label
            const categoryLabel = this.getCategoryLabel(params.categoryId);

            // Generate recommendation
            const recommendation = this.geteModeRecommendation(params.categoryId, position);
    ...
    private getCategoryLabel(categoryId: number): string {
        switch (categoryId) {
            ...
            case 1: return 'Stablecoins';
            case 2: return 'ETH-correlated';
            default: return `Category ${categoryId}`;
        }
    }

    private checkAssetCompatibility(position: any, categoryId: number): string[] {
    ...
    private geteModeRecommendation(categoryId: number, position: any): string {
        if (categoryId === 0) {
