    ...
    async handler(
        ...
        try {
            ...
            if (healthFactor.lt(1.2)) {
                throw new Error(`Health factor ${healthFactor.toFixed(2)} is too low. Supply more collateral before borrowing.`);
            }

            // Check available borrow capacity
            const availableBorrows = new BigNumber(accountData.availableBorrowsETH.toString()).dividedBy(1e18);
